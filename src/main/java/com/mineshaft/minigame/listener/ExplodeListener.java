    ...
    void ExplodeEvent(EntityExplodeEvent e) {
        for(Arena arena : minigame.getArenaManager().getArenas()) {
            if(arena.getGameType().equals(GameType.BEDWARS)&& e.getLocation().getWorld().getName().equals(arena.getWorld().getName())) {
                ...
                e.setCancelled(true);
                if(e.getEntity().getType().equals(EntityType.TNT)) {
                    arena.getWorld().createExplosion(e.getLocation(),4,false,false);
                    e.getLocation().getWorld().spawnParticle(Particle.EXPLOSION, e.getLocation(), 25, 1.25, 1.25 ,1.25);
                } else if(e.getEntity().getType().equals(EntityType.FIREBALL)) {
                    arena.getWorld().createExplosion(e.getLocation(),2,true,false);
                    e.getLocation().getWorld().spawnParticle(Particle.EXPLOSION, e.getLocation(), 15, 0.8, 0.8 ,0.8);
                }
